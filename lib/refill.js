...
export async function submitH3Refill(opts = {}) {
  ...
  const nSlots = Array.isArray(library.slots) ? library.slots.length : 0;
  const nPending = library.pending && typeof library.pending === "object"
    ? Object.keys(library.pending).length
    : 0;
  const slot = nSlots + nPending;
  const generation = Math.max(
    typeof library.promptSeq === "number" ? library.promptSeq : 0,
    slot,
  );
