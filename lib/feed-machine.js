...
export function createFeedMachine(opts = {}) {
  ...
  function hydrateFrom(data) {
    ...
    for (const rec of list) {
      ...
      if (!Number.isInteger(slot) || slot < 0 || slot >= poolSize) continue;
  ...
  function oldestClip() {
    ...
    for (const clip of slots.values()) {
      if (!best || clip.createdAt < best.createdAt) best = clip;
    }
    return best;
  }
  ...
  async function getFeed(index, extra = {}) {
    ...
    const current = slots.get(slotOf(feedIndex, poolSize));
    if (
      waitFallbackMs > 0 &&
      current &&
      !current.t2vUrl &&
  ...
  function getByIndex(index) {
    ...
    return serialize(slots.get(slotOf(feedIndex, poolSize)), feedIndex);
  }
  ...
  return {
    ...
    poolStatus,
    snapshot,
    raw: (indexOrSlot) => slots.get(slotOf(indexOrSlot, poolSize)),
    startCounts: counts,
  };
}
