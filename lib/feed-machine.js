...
/** Fisher–Yates permutation of 0..size-1. Same seed => same order. */
export function shuffledSlots(size, seed) {
...
export function slotOf(feedIndex, poolSize = POOL_SIZE, order) {
...
export function createFeedMachine(opts = {}) {
  ...
  function ensureWindow(feedIndex, modelName, order) {
    ...
    for (let i = start; i <= start + prefetch; i++) {
      const slot = slotOf(i, wrap, order);
      if (!slots.has(slot)) {
  ...
  async function getFeed(index, extra = {}) {
    ...
    const wrap = wrapModulus();
    const order =
      extra.seed != null && extra.seed !== "" ? shuffledSlots(wrap, extra.seed) : null;
    const window = ensureWindow(feedIndex, modelName, order);
    const current = slots.get(slotOf(feedIndex, wrap, order));
    if (
      waitFallbackMs > 0 &&
      current &&
      !current.t2vUrl &&
